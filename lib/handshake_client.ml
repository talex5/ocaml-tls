...
let answer_certificate_RSA state session cs raw log =
  ...
  in
  validate_chain cfg.authenticator cs session peer_name keytype usage >>= fun session ->
  ...
  peer_rsa_key (List.hd session.peer_certificate) >|= fun pubkey ->
...
let answer_server_key_exchange_DHE_RSA state session kex raw log =
  ...
  verify_digitally_signed state.protocol_version leftover sigdata session.peer_certificate
  >>= fun () ->
